...
def load_uploaded_dataset(uploaded_file):
    try:
        return pd.read_csv(uploaded_file), "Uploaded CSV dataset"
    except Exception:
        uploaded_file.seek(0)
        return pd.read_table(uploaded_file), "Uploaded table dataset"
...
def train_and_evaluate(X, y, var_smoothing):
...
try:
    ...
    default_new_data = ",".join(str(value) for value in X.iloc[0].tolist())
    new_data = st.sidebar.text_input(
        "Enter New Data (comma-separated values):",
        default_new_data,
    )

    if new_data:
        try:
            new_data_list = [float(x.strip()) for x in new_data.split(",")]
            if len(new_data_list) != len(X.columns):
                st.error(f"Please enter {len(X.columns)} values, one for each feature.")
            else:
                new_data_df = pd.DataFrame([new_data_list], columns=X.columns)
                prediction = model.predict(new_data_df)[0]
                st.success(f"Prediction: {prediction}")
        except ValueError:
            st.error("Invalid input format. Please enter comma-separated values of numbers.")
